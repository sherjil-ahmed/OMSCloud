...
{
    public class ActiveChatClients
    {
        private static ActiveChatClients instance = new ActiveChatClients();
        private Dictionary<long, List<string>> clientList; //Key = ProfileID, Value List of SignalR Client Ids
        object mutex;

        private ActiveChatClients()
        {
            clientList = new Dictionary<long, List<string>>();
            mutex = new object();
        }

        public static ActiveChatClients Instance
        {
            get
            {
                return instance;
