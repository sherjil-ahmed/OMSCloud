...
public static class Security_ExtendedMethods
{
    ...
    #region Registration
    private static long RegisterUser(this ControllerBase controller, RegisterViewModel model, ApplicationUserManager userMngr, out List<string> _errors)
    {
        long _retVal = -1;
    ...
    public static SecurityStatus Register(this ControllerBase controller, RegisterViewModel model, ApplicationUserManager userMngr, ApplicationSignInManager signInMngr, out List<string> _errors)
    {
        SecurityStatus _retVal = SecurityStatus.Failure;
        try
        {
            //Logic driven by settings defined in the application’s configuration file...
            long _userId = Security_ExtendedMethods.RegisterUser(controller, model, userMngr, out _errors);
            if (_userId > -1)
            {
                model.Id = _userId;
                if (userMngr != null)
    ...
    #region Login
    public static SecurityStatus Login(this ControllerBase controller, LoginViewModel model, ApplicationUserManager userMngr, ApplicationSignInManager signInMngr, out List<string> _errors)
    {
        SecurityStatus _retVal = SecurityStatus.Failure;
        _errors = new List<string>();
        try
        {
            ...
            if (user != null)
            {
                ...
                if (userMngr.IsLockedOut(user.Id))
                {
                    ...
                    return SecurityStatus.LockedOut;
                }
                ...
                else
                {
                    ...
                    if ((IsAccountVerificationRequired && DeviceType == c_EmailCode) || (Is2FAEnabled && DeviceType == c_EmailCode))
                    {
                        ...
                        if (!userMngr.IsEmailConfirmed(user.Id))
                        {
                            ...
                            return SecurityStatus.EmailUnconfirmed;
                        }
                    }
                    //else if (Is2FAEnabled && DeviceType == c_PhoneCode)
                    else if ((IsAccountVerificationRequired && DeviceType == c_PhoneCode) || (Is2FAEnabled && DeviceType == c_PhoneCode))
                    {
                        if (!userMngr.IsPhoneNumberConfirmed(user.Id))
                        {
                            ...
                            return SecurityStatus.PhoneNumberUnconfirmed;
                        }
                    }
                    ...
                    switch (_retVal)
                    {
                        ...
                        case SecurityStatus.RequiresVerification:
                        {
                            ...
                            if (user.Roles.Count > 0)
                            {
                                ...
                                RoleOptionPairControllerProxy proxyRoleOptionPairController = new RoleOptionPairControllerProxy();
                                List<RoleOptionPairModel> roleOptionPairModel = proxyRoleOptionPairController.GetByRoleIds(roleIds);
                                roleOptionPairModel = roleOptionPairModel.Where(x => x.Option.ModuleID == 1).ToList(); //2 is for Public Site and 1 is for Admin
                                if (roleOptionPairModel.Count > 0)
                                {
                                    ApplicationSession.RoleOptionPairModelList = roleOptionPairModel;
                                }
    ...
    public static bool SendOTP2Phone(this ControllerBase controller, ApplicationUserManager _userMngr, long _userId, string _phoneNumber)
    {
        bool _retVal = false;
    ...
    public static bool SendOTP2Email(this ControllerBase controller, ApplicationUserManager _userMngr, long _userId, string _email)
    {
        bool _retVal = false;
    ...
    public static SecurityStatus RequestAccountVerification(this ControllerBase controller, long _userId, string _email, ApplicationUserManager userMngr, out IEnumerable<string> _errors)
    {
        SecurityStatus _retVal = SecurityStatus.Failure;
        _errors = new List<string>();
