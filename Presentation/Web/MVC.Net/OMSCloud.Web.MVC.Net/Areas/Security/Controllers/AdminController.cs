...
namespace OMSCloud.Web.MVC.Net.Areas.Security.Controllers
{
    ...
    public class AdminController : BaseMvcController
    {
        ...
        public ActionResult UserCreate(RegisterViewModel model)
        {
            ...
            ModelState[nameof(model.UserName)].Errors.Clear();
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    //model.Id is updated inside Register Method Below (if successful to register user)
                    SecurityStatus _retVal = this.Register(model, this.UserManager, this.SignInManager, out _errors);
                    ApplicationUserManager.AddUser2Role(model.Id, (long)DBSystemRoleEnum.AdminPortal);                    
                    ...
                    switch (_retVal)
                    {
                        ...
                        case SecurityStatus.RequiresAccountActivation:
                        {
                            ...
                            ViewBag.Message += " Email has been sent for verification at " + model.Email;
                            break;
                        }
                        case SecurityStatus.EmailVerification:
                        {
                            ViewBag.Message += " Email verification is required : " + model.Email;
                            break;
                        }
                        case SecurityStatus.PhoneVerification:
                        {
                            ViewBag.Message += " SMS has been sent for verification at mobile # " + model.Mobile;
                            break;
                        }
                    }
            ...
            if (model.Id > 0)
            {
                return RedirectToAction("UserDetails", new RouteValueDictionary(new { id = model.Id }));
            }
            else
            {
        ...
        {
            model.UserName = model.Email;
            ApplicationUserManager.UpdateUser(model);
        ...
        public ActionResult PermissionsImport()
        {
            ...
            var _controllerMethods = _controllerTypes.ToDictionary(controllerType => controllerType,
                    ...
                    .Where(m => m.DeclaringType.Name == controllerType.Name && m.IsSpecialName == false)
                    );
