...
namespace OMSCloud.Web.MVC.Net.Areas.Security
{
        ...
        private bool Send2HotmailAccount(IdentityMessage message)
        {
            ...
            try
            {
                using (var mail = new MailMessage())
                {
                    ...
                    smtp.EnableSsl = true;// true; // Outlook.com and Gmail require SSL
    ...
    public class SmsService : IIdentityMessageService
    {
        public Task SendAsync(IdentityMessage message)
        {
            SMSGatewayControllerProxy proxy = new SMSGatewayControllerProxy();
    ...
    public class ApplicationUserManager : UserManager<ApplicationUser, long>
    {
        ...
        public static bool UpdateUser(ApplicationUser appUser)
        {
            bool _retVal = false;
            try
            {
                using (SecurityDbContext db = SecurityDbContext.CreateInstance())
                {
                    ApplicationUser _user2Modify = GetUser(db, appUser.Id);

                    db.Entry(_user2Modify).Entity.UserName = appUser.UserName;
                    db.Entry(_user2Modify).Entity.Email = appUser.Email;
                    db.Entry(_user2Modify).Entity.Firstname = appUser.Firstname;
                    db.Entry(_user2Modify).Entity.Lastname = appUser.Lastname;
                    db.Entry(_user2Modify).Entity.PhoneNumber = appUser.PhoneNumber;
                    db.Entry(_user2Modify).Entity.PhoneNumberConfirmed = appUser.PhoneNumberConfirmed;
                    db.Entry(_user2Modify).Entity.EmailConfirmed = appUser.EmailConfirmed;
                    db.Entry(_user2Modify).Entity.LockoutEnabled = appUser.LockoutEnabled;
                    db.Entry(_user2Modify).Entity.AccessFailedCount = appUser.AccessFailedCount;
                    db.Entry(_user2Modify).Entity.Inactive = appUser.Inactive;
                    db.Entry(_user2Modify).Entity.TwoFactorEnabled = appUser.TwoFactorEnabled;
                    db.Entry(_user2Modify).Entity.LastModified = System.DateTime.Now;
                    db.Entry(_user2Modify).State = EntityState.Modified;
                    db.SaveChanges();
