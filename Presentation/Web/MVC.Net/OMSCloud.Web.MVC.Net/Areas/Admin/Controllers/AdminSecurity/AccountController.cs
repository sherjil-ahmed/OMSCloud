    ...
    public class AccountController : BaseMvcController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    SecurityStatus _retVal = this.Login(model, this.UserManager, this.SignInManager, out _errors);
                    switch (_retVal)
                    {
                        case SecurityStatus.Success:
                        {
                            //string[] roles = Roles.GetRolesForUser(this.User.Identity.GetUserName());
                ...
                catch (System.Web.Http.HttpResponseException rEx)
                {
                    var additionalExceptionDetail = rEx.Response.Content.ReadAsStringAsync().Result;
                    AddErrors(new IdentityResult(rEx.Message + Environment.NewLine + additionalExceptionDetail));
                }
                catch (Exception ex)
                {
        ...
        public ActionResult OTP4PhoneVerification(int UserId, string phoneNumber, bool displayError = false)
        {
            ...
            if (displayError)
                AddErrors(new IdentityResult(string.Format(Security_ExtendedMethods.c_AccountPhoneNumberUnconfirmed, phoneNumber)));
        ...
        public ActionResult OTP4PhoneVerification(VerifyOTPPhoneViewModel model)
        {
            ...
            SecurityStatus result = Security_ExtendedMethods.VerifyOTP4Phone(model.UserId, model.PhoneNumber, model.Code, this.UserManager, this.SignInManager, out _errors);
            if (result == SecurityStatus.Success)
            {
        ...
        public ActionResult OTP4EmailVerification(TOTP4EmailViewModelPost model)
        {
            ...
            IEnumerable<string> _errors = new string[0];
            bool result = Security_ExtendedMethods.VerifyOTP4Email(model.UserId, model.SecurityPIN, this.UserManager, this.SignInManager, out _errors);
            if (result)
            {
                return RedirectToAction("Index", "Home");
            }
        ...
        public ActionResult RequestEmailVerification(TOTP4EmailViewModelGet model)
        {
            ...
            IEnumerable<string> _errors = new string[0];
            SecurityStatus result = this.RequestAccountVerification(model.UserId, model.Email, this.UserManager, out _errors);
            if (result == SecurityStatus.RequiresAccountActivation)
            {
        ...
        public async Task<ActionResult> SendSecurityCode(SendCodeViewModel model)
        {
            ...
            if (model.SelectedProvider == Security_ExtendedMethods.c_PhoneCode)
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    SecurityStatus _retVal = this.Register(model, this.UserManager, this.SignInManager, out _errors);
                    switch (_retVal)
                    {
                        case SecurityStatus.Success:
                            {
                                ApplicationUserManager.AddUser2Role(model.Id, 3);
                                var profile = new ProfileControllerProxy();
                                var profileId = profile.Put(new ProfileModel
                                {
                                    UserID = model.Id,
                                ...
                                ViewBag.Message = profileId.HasValue ?
                                    "Your account has been created successfully.  You can now continue and login..." :
                                    "Your account has been created successfully. But there is problem when creating User Profile.";
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = user.Id, code = WebUtility.UrlEncode(code) }, protocol: Request.Url.Scheme);
        ...
        {
            ViewBag.RequiredLength = Security_ExtendedMethods.GetConfigSettingAsInt(Security_ExtendedMethods.cKey_PasswordRequiredLength, 6);
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            var result = await UserManager.ResetPasswordAsync(user.Id, WebUtility.UrlDecode(model.Code), model.Password);
            if (result.Succeeded)
            {
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
